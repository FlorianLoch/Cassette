...
.container
  .jumbotron.mt-4
    ...
    p What data do we store? Why? What do we do with it? The following list should clarify these questions.
    ul
      li
        p Your playing states are stored in a MongoDB database hosted by
        |
        a(href="https://www.mongodb.com/cloud/atlas") MongoDB Inc
        |
        | in Ireland.
    p There is a DPA (Data Processing Addendum) with Cloudflare; MongoDB incorporated data protection policies into their terms of service already - a DPA is therefore not required. TODO: What about Heroku? As you are a Spotify customer already and this service is solely acting on your behalf the terms of service and the privacy policy of Spotify apply to your (indirect) interactions with their platform.
    ...
    .row.mx-auto
      ...
      b-button(@click="giveConsent" variant="primary" :disabled="$api.isConsentCookieValid()") Accept
      b-button.ml-1(@click="exportData" variant="info") Export my data
      b-button.ml-1(@click="deleteData" variant="danger") Delete my data
</template>
